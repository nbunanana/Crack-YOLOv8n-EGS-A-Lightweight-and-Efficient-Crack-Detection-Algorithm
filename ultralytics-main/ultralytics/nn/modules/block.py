...
from .conv import Conv, Conv_GN, DWConv, GhostConv, LightConv, RepConv, autopad
...
class C2f_EMA(C2f):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        """Initialize C2f_EMA module."""
        super().__init__(c1, c2, n, shortcut, g, e)
        self.attn = EMA(c2)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through C2f_EMA layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
...
class C2f_GN(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions and GroupNorm."""

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        """Initialize a CSP bottleneck with GroupNorm.
        ...
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv_GN(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv_GN((2 + n) * self.c, c2, 1)
        self.m = nn.ModuleList(Bottleneck_GN(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through C2f_GN layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
...
class C2f_EMA_GN(nn.Module):
    """C2f module with GroupNorm and EMA attention."""

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        """Initialize C2f_EMA_GN module with GroupNorm convolutions and EMA attention."""
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv_GN(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv_GN((2 + n) * self.c, c2, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through C2f_EMA_GN layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.attn(self.cv2(torch.cat(y, 1)))
...
class Bottleneck_GN(nn.Module):
    """Bottleneck with GroupNorm."""

    def __init__(
        self, c1: int, c2: int, shortcut: bool = True, g: int = 1, k: tuple[int, int] = (3, 3), e: float = 0.5
    ):
        """Initialize a bottleneck module with GroupNorm."""
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv_GN(c1, c_, k[0], 1)
        self.cv2 = Conv_GN(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2
